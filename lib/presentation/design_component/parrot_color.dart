import 'package:flutter/material.dart';

class ParrotColor {
  /*gray*/
  static const Color gray900 = Color(0xFF121212);
  static const Color gray800 = Color(0xFF222222);
  static const Color gray700 = Color(0xFF424242);
  static const Color gray600 = Color(0xFF5D5D5D);
  static const Color gray500 = Color(0xFF888888);
  static const Color gray400 = Color(0xFF999999);
  static const Color gray300 = Color(0xFFB2B2B2);
  static const Color gray200 = Color(0xFFC0C0C0);
  static const Color gray100 = Color(0xFFDBDBDB);
  static const Color gray50 = Color(0xFFEEEEEE);

  /*red*/
  static const Color red900 = Color(0xFF780700);
  static const Color red800 = Color(0xFFA40900);
  static const Color red700 = Color(0xFFCE0B00);
  static const Color red600 = Color(0xFFF00D00);
  static const Color red500 = Color(0xFFFF3519);
  static const Color red400 = Color(0xFFFF705D);
  static const Color red300 = Color(0xFFFF988A);
  static const Color red200 = Color(0xFFFFC7BF);
  static const Color red100 = Color(0xFFFFEBE8);
  static const Color red50 = Color(0xFFFFF5F4);

  /*blue*/
  static const Color blue500 = Color(0xFF2553F8);
  static const Color blue100 = Color(0xFFE9EEFE);
  static const Color blue50 = Color(0xFFF4F6FF);

  /*green*/
  static const Color green500 = Color(0xFF4BD984);
  static const Color green50 = Color(0xFFF6FDF9);

  /*yellow*/
  static const Color yellow500 = Color(0xFFFFCF52);
}
